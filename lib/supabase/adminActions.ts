...
export async function saveProfileData(data: FullProfileData): Promise<{ success: boolean; message: string }> {
  ...
  try {
    ...
    // 2. Save Life Events
    const { error: delLifeErr } = await supabase.from('life_events_warga_demo1').delete().eq('profile_id', pId);
    if (delLifeErr) errors.push(`life_events delete: ${delLifeErr.message}`);
    if (lifeEvents && lifeEvents.length > 0) {
      const { error: insLifeErr } = await supabase.from('life_events_warga_demo1').insert(lifeEvents.map((item, idx) => ({
    ...
    // 3. Save Works
    const { error: delWrkErr } = await supabase.from('works_warga_demo1').delete().eq('profile_id', pId);
    if (delWrkErr) errors.push(`works delete: ${delWrkErr.message}`);
    if (works && works.length > 0) {
      const { error: insWrkErr } = await supabase.from('works_warga_demo1').insert(works.map((item, idx) => ({
        id: isValidUUID(item.id) ? item.id : crypto.randomUUID(),
        profile_id: pId,
        title: item.title || '',
    ...
    // 4. Save Articles
    const { error: delArtErr } = await supabase.from('articles_warga_demo1').delete().eq('profile_id', pId);
    if (delArtErr) errors.push(`articles delete: ${delArtErr.message}`);
    if (articles && articles.length > 0) {
      const { error: insArtErr } = await supabase.from('articles_warga_demo1').insert(articles.map((item, idx) => ({
        id: isValidUUID(item.id) ? item.id : crypto.randomUUID(),
        profile_id: pId,
        title: item.title || '',
    ...
    // 5. Save Testimonials
    const { error: delTstErr } = await supabase.from('testimonials_warga_demo1').delete().eq('profile_id', pId);
    if (delTstErr) errors.push(`testimonials delete: ${delTstErr.message}`);
    if (testimonials && testimonials.length > 0) {
      const { error: insTstErr } = await supabase.from('testimonials_warga_demo1').insert(testimonials.map((item, idx) => ({
        id: isValidUUID(item.id) ? item.id : crypto.randomUUID(),
        profile_id: pId,
    ...
    const { error: delIniErr } = await supabase.from('initiatives_warga_demo1').delete().eq('profile_id', pId);
    if (delIniErr) errors.push(`initiatives delete: ${delIniErr.message}`);
    if (initiatives && initiatives.length > 0) {
      const { error: insIniErr } = await supabase.from('initiatives_warga_demo1').insert(initiatives.map((item, idx) => ({
        id: isValidUUID(item.id) ? item.id : crypto.randomUUID(),
        profile_id: pId,
        title: item.title || '',
    ...
    // 7. Save Gallery
    const { error: delGalErr } = await supabase.from('gallery_warga_demo1').delete().eq('profile_id', pId);
    if (delGalErr) errors.push(`gallery delete: ${delGalErr.message}`);
    if (gallery && gallery.length > 0) {
      const { error: insGalErr } = await supabase.from('gallery_warga_demo1').insert(gallery.map((item, idx) => ({
        id: isValidUUID(item.id) ? item.id : crypto.randomUUID(),
        profile_id: pId,
        title: item.title || '',
        image_url: item.image_url || '',
...
export async function deleteProfileBySlug(slug: string): Promise<{ success: boolean; message: string }> {
  ...
  try {
    const supabase = createClient();
    await supabase.from('profiles_warga_demo1').delete().eq('slug', slug);
  } catch {
