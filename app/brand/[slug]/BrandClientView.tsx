...
import { BrandProfile, brandProfiles, BrandProduct, BrandMilestone } from '../../../lib/supabase/brandData';
import { getBrandFullData } from '../../../lib/supabase/brandAdminActions';
...
export default function BrandClientView({ brand }: BrandClientViewProps) {
  ...
  const [currentBrand, setCurrentBrand] = useState<BrandProfile>(brand);

  // Dynamic fetch from Supabase on mount
  useEffect(() => {
    let isMounted = true;
    const loadDynamic = async () => {
      try {
        const fresh = await getBrandFullData(brand.slug);
        if (fresh && isMounted) {
          setCurrentBrand(fresh);
        }
      } catch {
        // keep initial props
      }
  ...
  const relatedBrands = Object.values(brandProfiles)
    .filter(b => b.lensSlug === currentBrand.lensSlug && b.slug !== currentBrand.slug)
    .slice(0, 3);

  // Extract channels
  const officialStores = currentBrand.channels?.officialStore || [];
  const publicSpaces = currentBrand.channels?.publicSpaces || [];

  // Prepare bio paragraphs
  const bioParagraphs: string[] = (currentBrand.bio_paragraphs && currentBrand.bio_paragraphs.length > 0)
    ? currentBrand.bio_paragraphs
    : [];

  // Prepare milestones
  const milestones: BrandMilestone[] = (currentBrand.milestones && currentBrand.milestones.length > 0)
    ? currentBrand.milestones
    : [];

  // Prepare products
  const products: BrandProduct[] = (currentBrand.flagshipProducts && currentBrand.flagshipProducts.length > 0)
    ? currentBrand.flagshipProducts
    : [];
  ...
  return (
    <div className={`min-h-screen selection:bg-[#E11D48] selection:text-white font-sans ${isDarkMode ? 'magazine-dark text-[#F5EFEB] bg-[#07090E]' : 'magazine-light text-[#07090E] bg-white'}`}>
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 relative z-10">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pb-6 sm:pb-8 mb-8 sm:mb-12 border-b border-white/10 text-xs font-mono">
            <div className="flex items-center gap-3">
              <Link 
                href={`/brand-lokal/${currentBrand.lensSlug}`} 
              ...
              >
                ...
                <span>Laci: {currentBrand.lensName}</span>
              </Link>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-center">
            ...
            >
              ...
              <p className="font-mono text-xs sm:text-sm text-[#8E95A5] uppercase tracking-wider mb-3 flex items-center gap-2">
                ...
                <span>{currentBrand.lensName} | {currentBrand.origin}</span>
              </p>
              ...
              {/* Pull-Quote (Rendered only if data exists) */}
              {(currentBrand.quote || currentBrand.coreFinding) && (
                <div className="border-l-2 border-[#E11D48] pl-4 sm:pl-5 py-1 mb-8 bg-white/[0.02]">
                  ...
                  <p className="font-editorial italic text-base sm:text-lg md:text-xl text-[#F8F5EE]/90 leading-snug descender-safe">
                    &ldquo;{currentBrand.quote || currentBrand.coreFinding}&rdquo;
                  </p>
                </div>
              )}

              {/* Two-Row Channels Layout (Toko Resmi & Sosmed Terpisah Rapi) */}
              <div className="space-y-3.5 pt-2">
                {/* BARIS 1: Toko Resmi (Hanya tampil jika ada data) */}
                {officialStores.length > 0 && (
                  <div className="flex flex-wrap items-center gap-2.5">
                    <span className="text-[10px] font-mono text-[#8E95A5] uppercase tracking-wider block mr-1">
                      Toko Resmi:
                    </span>
                    {officialStores.map((store, i) => (
                      <a
                        ...
                        rel="noreferrer"
                        title={store.label}
                        className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-white/5 hover:bg-[#E11D48] text-white/80 hover:text-white border border-white/10 hover:border-[#E11D48] text-xs font-mono uppercase tracking-wider transition-all duration-200 tactile-btn"
                      >
                ...
                {/* BARIS 2: Media Sosial & Aksi Berkas (Hanya tampil jika ada data) */}
                <div className="flex flex-wrap items-center gap-3">
                  {publicSpaces.length > 0 && (
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {publicSpaces.map((pub, i) => {
                        const isMail = pub.url.startsWith('mailto:');
                        const isTwitter = pub.platform.toLowerCase().includes('twitter') || pub.platform.toLowerCase().includes('x');
                        const isYoutube = pub.platform.toLowerCase().includes('youtube');
                  ...
                  <button 
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white hover:text-black text-white font-mono text-xs uppercase tracking-wider font-semibold border border-white/20 transition-all duration-200 tactile-btn"
                  >
                    {copied ? (
                      <>
      ...
      {/* (Hanya tampil jika ada paragraf narasi) */}
      {/* ========================================================================= */}
              ...
              <div className="lg:col-span-3 space-y-4">
                ...
                <div className="hidden lg:block pt-6 border-t border-black/10 space-y-3 font-mono text-xs">
                  <div>
                    ...
                    <span className="font-bold text-[#E11D48]">{currentBrand.lensName}</span>
                  </div>
                  <div>
                    <span className="text-[#07090E]/50 block uppercase text-[10px]">Asal Daerah</span>
                    <span className="font-bold text-[#07090E]">{currentBrand.origin}</span>
                  </div>
                  <div>
                    <span className="text-[#07090E]/50 block uppercase text-[10px]">Tingkat Kurasi</span>
                    <span className="font-bold text-[#07090E]">{currentBrand.curationTier}</span>
                  </div>
                  <div>
      ...
      {/* (Hanya tampil jika ada quote atau milestones) */}
      {/* ========================================================================= */}
      {(currentBrand.quote || currentBrand.sovereigntyMeaning || milestones.length > 0) && (
          ...
          <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 relative z-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
              ...
              <div className="lg:col-span-5 space-y-6">
                ...
                {(currentBrand.quote || currentBrand.sovereigntyMeaning) && (
      ...
      {/* (Hanya tampil jika ada karya/produk) */}
      {/* ========================================================================= */}
      {products.length > 0 && (
              ...
              {products.map((p, i) => (
                ...
                >
                  ...
                  <div className="pt-4 border-t border-white/15 flex items-center justify-between gap-4">
                    ...
                    >
                      <span>{p.action_text || 'Beli di Toko Resmi'}</span>
      ...
      {/* ========================================================================= */}
      {/* ACT 5: BRAND TERKAIT / LACI KURASI SEJENIS */}
      {/* (Hanya tampil jika ada brand terkait) */}
      {/* ========================================================================= */}
        ...
        <section className="w-full bg-[#07090E] text-[#F5EFEB] py-20 sm:py-28 border-b border-white/10">
          <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12 pb-6 border-b border-white/10">
              ...
              >
                <span>Lihat Semua di Laci {currentBrand.lensName} &rarr;</span>
              </Link>
            </div>
