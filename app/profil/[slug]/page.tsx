      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            ...
            <a href="#artikel" onClick={() => setIsMenuOpen(false)} className="hover:text-[#E11D48] transition-colors">04 // Artikel & Wawasan</a>
            <a href="#kata-warga" onClick={() => setIsMenuOpen(false)} className="hover:text-[#E11D48] transition-colors">05 // Kata Warga</a>
          ...
          >
            ...
            <div className="flex items-center gap-2 pt-2">
              {profile.social_links.youtube && (
                <a href={profile.social_links.youtube} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${isDarkMode ? 'border-white/10 hover:border-[#E11D48] text-[#8A93A8]' : 'border-black/10 hover:border-[#E11D48] text-gray-600'}`}>
                  <Youtube className="w-4 h-4" />
                </a>
              )}
              {profile.social_links.twitter && (
                <a href={profile.social_links.twitter} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${isDarkMode ? 'border-white/10 hover:border-[#E11D48] text-[#8A93A8]' : 'border-black/10 hover:border-[#E11D48] text-gray-600'}`}>
                  <Twitter className="w-4 h-4" />
                </a>
              )}
              {profile.social_links.instagram && (
                <a href={profile.social_links.instagram} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${isDarkMode ? 'border-white/10 hover:border-[#E11D48] text-[#8A93A8]' : 'border-black/10 hover:border-[#E11D48] text-gray-600'}`}>
                  <Instagram className="w-4 h-4" />
                </a>
              )}
              {profile.social_links.spotify && (
                <a href={profile.social_links.spotify} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${isDarkMode ? 'border-white/10 hover:border-[#E11D48] text-[#8A93A8]' : 'border-black/10 hover:border-[#E11D48] text-gray-600'}`}>
                  <Radio className="w-4 h-4" />
                </a>
              )}
              {profile.social_links.email && (
                <a href={profile.social_links.email} className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${isDarkMode ? 'border-white/10 hover:border-[#E11D48] text-[#8A93A8]' : 'border-black/10 hover:border-[#E11D48] text-gray-600'}`}>
              ...
              {articles.map((art, idx) => (
                <div key={idx} className={`p-6 sm:p-8 rounded-3xl border space-y-4 flex flex-col justify-between ${cardClass}`}>
                  ...
                  <div className="pt-2 flex justify-end">
                    <a href={art.link_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-bold text-[#E11D48] hover:underline">
          ...
          <section id="kata-warga" className="space-y-8 scroll-mt-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {testimonials.map((item, idx) => (
                <div key={idx} className={`p-6 rounded-3xl border flex flex-col justify-between space-y-4 ${cardClass}`}>
          ...
          <section id="produk" className="space-y-8 scroll-mt-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {initiatives.map((prod, idx) => (
                <div key={idx} className={`p-6 rounded-3xl border flex flex-col justify-between space-y-5 ${cardClass}`}>
                  <div className="space-y-2">
                    <span className="text-[10px] font-mono text-[#E11D48] uppercase font-semibold">{prod.category}</span>
                    <h3 className="font-display uppercase text-lg text-inherit">{prod.title}</h3>
                    <p className={`text-xs leading-relaxed ${mutedText}`}>{prod.description}</p>
                  </div>
                  <a href={prod.link_url} target="_blank" rel="noreferrer" className="w-full py-2.5 bg-[#E11D48] hover:bg-[#BE123C] text-white font-bold text-xs rounded-xl transition-colors flex items-center justify-center gap-1 shadow-sm shadow-[#E11D48]/30">
